...
class ExpoBonjourModule : Module() {
  ...
  override fun definition() = ModuleDefinition {
    ...
    Events("onStartScan", "onStopScan", "onDeviceFound", "onDeviceChange", "onDeviceRemoved", "onError")
    ...
    AsyncFunction("getIPAddress") { device: Map<String, Any?>, promise: Promise ->
      try {
        ...
        val targetKey = discoveredDevices.keys.find { key ->
          ...
          val infoFullName = "${info?.serviceName}.${info?.serviceType}${info?.host?.hostName ?: ""}"
  ...
  private fun resolveService(serviceInfo: NsdServiceInfo, callback: (Map<String, Any?>?) -> Unit) {
